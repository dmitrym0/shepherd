...
// Add your own opencode plugins beside this file instead of editing it.
//
...
// Changes: herdr->shepherd names for the socket methods, params and env vars.
// SHEPHERD_INTEGRATION_ID=opencode
...
// Track child sessions so their events cannot replace the terminal's root session.
...
function reportSession(sessionID) {
  ...
  return request("agent.report_session", { agent_session_id: sessionID });
}
...
function reportState(state, sessionID) {
  ...
  return request("agent.report_agent", params);
}
    ...
    event: async ({ event }) => {
      ...
      switch (type) {
        case "session.created":
          ...
          // TUI plugin separately reports the root selected in this terminal.
          reportedRootSessionID = sessionID;
          break;
